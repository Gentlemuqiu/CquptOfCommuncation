...
public class JubaoController {
    ...
    public Result<?> save(@RequestBody Jubao jubao) {
        ...
        Jubao one = jubaoMapper.selectOne(Wrappers.<Jubao>lambdaQuery()
                .eq(Jubao::getCommentId, commentId));
        if (one != null) {
    ...
    public Result<?> findPage(
            ...
            Integer pageSize) {
        LambdaQueryWrapper<Jubao> query =
                Wrappers.<Jubao>lambdaQuery().orderByDesc(Jubao::getId);
        IPage<Jubao> page =
                jubaoMapper.selectPage(new Page<>(pageNum, pageSize), query);
